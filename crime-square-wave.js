...
const stoploss_counter = STOPLOSS_COUNTER
const wallet_percentage = WALLET_QUANTITY_PERCENTAGE
const wallet_divider= WALLET_DIVIDER
const ohlcv_period = OHLCV_PERIOD
const coin_fee_autocharge = COIN_FEE_AUTOCHARGE
const coin_fee = COIN_FEE
const coin_fee_charge = COIN_FEE_CHARGE
const coin_fee_threshold = COIN_FEE_THRESHOLD
const ob_level = OB_LEVEL
const ob_filter = ORDERBOOK_FILTER
const direction = TREND_DIRECTIONL
...
const fibonacci_ohlcv_period = FIBONACCI_OHLCV_PERIOD
    ...
    for (var j=0;j<assets.length;j++) {
      ...
      if (BB_FILTER) {
        ...
        */
        ...
        //current BB
        var bb_lower = functions.priceToPrecision(exchange.id,markets, asset,bb[3].lower)
        var bb_middle = functions.priceToPrecision(exchange.id,markets, asset,bb[3].middle)
        var bb_upper = functions.priceToPrecision(exchange.id,markets, asset,bb[3].upper)
        //previous BB
        var previous_bb_lower = functions.priceToPrecision(exchange.id,markets, asset,bb[4].lower)
        var previous_bb_middle = functions.priceToPrecision(exchange.id,markets, asset,bb[4].middle)
        var previous_bb_upper = functions.priceToPrecision(exchange.id,markets, asset,bb[4].upper)
      ...
      if (ORDERBOOK_FILTER) {
        var obTrend = await checkOrderbook(exchange, asset,quantity)
        if (obTrend[0] && obTrend[1] ) {
          trend = "up"
        } else if (!obTrend[0] && !obTrend[1] ) { //false && false is fine; otherwise KO (=last)
          trend = "down"
        } else {
          log.bright.red(exchange.iso8601(Date.now()),exchange.id.green,algo_namespace.yellow, "skip the following assets",asset, "due orderbook analysis not aligned to the other TIs" )
          return;
...
async function initialize() {
  var exchange = exchanges[exchangeId];
  log.bright(exchange.iso8601 (Date.now ()),exchange.id.green,algo_namespace.yellow,  "initialize started")
...
async function handle_data () {
  var exchange = exchanges[exchangeId];
  log.bright(exchange.iso8601 (Date.now ()),exchange.id.green,algo_namespace.yellow, "handle_data started")
  //return  new Promise (async (resolve, reject) => {
    try {
      //results::= [coin_prices_quote_volume, coin_prices_base_volume, coin_last, assets]
      var results = await fetchVolumes(exchanges[exchangeId], coin_pair,symbol, symbols, price_changed,2)
      ...
      log.bright.yellow(exchange.iso8601 (Date.now ()),exchange.id.green,algo_namespace.yellow,"assets", assets)
